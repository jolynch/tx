...
- **Transport**: one TCP connection per command by default, server closes after completion. A PROBE with `keep-alive=auto` upgrades the connection to a kept-alive session (server grants `keep-alive-ms` in the response) that serves multiple commands, with client heartbeats at ¼ of the granted window and server-side idle reaping after the keep-alive window (`--idle-timeout`, default 60s; 0 disables). Heartbeats do not count as `--exit-after` activity.
...
**TCP connection pool.** `client_tcp.go` maintains an async-refilled pool (`tcpConnPool`) sized to the configured concurrency plus a 25% headroom (`warmTCPPoolTarget`). When the server grants keep-alive (PROBE `keep-alive=auto` request → `keep-alive-ms` response, cached in `Client.keepAliveMS`), pooled connections are `sessionTCPConn` sessions: a response consumed through its terminal status line is marked via `markReusable`/`finishManagedTCPConn(clean=true)` and recycled back into the pool (`recycleManagedTCPConn`), and a background loop heartbeats idle pooled connections at ¼ of the granted window with zero-payload PROBEs (`heartbeatSessionConn`), evicting dead ones; borrowers additionally peek for pending EOF (`sessionConnAlive`) before reuse. Without a grant (or with `Client.DisableKeepAlive`), connections are single-use as before. Borrowed connections are wrapped in a `managedTCPConnCloser`. When the pool is empty the client falls back to a synchronous single-use `dial` and calls `clientMetrics.IncSyncConnectionFallback()`, exposed via `MetricSnapshot().SyncConnectionCount`. `readTCPStatus` reads the `OK`/`ERR` terminal line; `readTCPLine` reads an arbitrary line up to `maxTCPLineBytes`.
