...
func ParseRequest(payload []byte) (Request, error) {
	...
	case VerbPROBE:
		...
		for !c.eof() {
			...
			switch key {
			case "cpu", "io-depth", "probe-bytes", "cts0", "sts0", "sts1", "wmem", "txferid", "obs-link-mbps", "gentle-cpu-pct", "gentle-bw-pct", "keep-alive", "keep-alive-ms", "target-request-bytes", "max-request-bytes", "max-sync-request-bytes":
				param[key] = val
			default:
				// Unknown keys are ignored for forward compatibility.
			}
